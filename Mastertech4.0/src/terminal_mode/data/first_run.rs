...
impl <'a>TerminalApp<'a> {
    pub fn first_run(&mut self) -> anyhow::Result<(), anyhow::Error> {
        ...
        if let Ok(svc_data) = service_data {
            ...
            self.ctx.client_title = url_string.clone();
            ...
            self.ctx.client_uuid = RecordId::from_table_key(
        ...
        match loaded_data {
            Some(login) => {
                let tx = self.ctx.db_tx.clone();
                tokio::spawn(async move {
                    let _ = tx.try_send(Database::new(login.username, login.password, None).await);
                });
            },
            None => self.ctx.app_state_tx.try_send(AppState::Login)?
        }
        Ok(())
    }
}