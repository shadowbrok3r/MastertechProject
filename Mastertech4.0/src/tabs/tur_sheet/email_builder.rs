...
pub struct TaskAssignee{
 ...
 pub fn email_builder(info: Info) -> String {
  ...
  let _hdd_test = info.hdd_test;
  let _ram_test = info.ram_test;
  let _ssd_test = info.ssd_test;
  let checkin_notes = info.checkin_notes;
  let recommendations = info.recommendations;
  let specs = info.specs;
  let _cps = info.cps;
  let _cust_code = info.cust_code;
  let _doc_alias = info.doc_alias;
  ...
  let _total_inv_num = info.total_inv_num;
  let phone1 = info.phone1;
  let phone2 = info.phone2;
  let final_disk = info.final_disk;
  let _salesman = info.salesman;
  let _checkin_rep = info.checkin_rep;
  let _technician = info.technician;
  let _extra_customer_info = info.extra_customer_info;
