...
use crate::{events::action_handler::{get_event_sender, WidgetButton, WidgetEvent, WidgetId}, fx::{effect::{animated_border, UniqueEffectId}, EffectStage}, styling::{Theme, TURQUOISE, APP_BACKGROUND, THEME}};
        ...
        match mouse_event.kind {
            MouseEventKind::Down(MouseButton::Left) => {
                if area.contains(mouse_position) {
                    ...
                    let _ = self.event_sender.try_send(WidgetEvent::ButtonClick { widget_id: self.id.clone(), button: WidgetButton::Left, source: self.source.clone() });
            ...
            MouseEventKind::Down(MouseButton::Right) => {
                if area.contains(mouse_position) {
                    let _ = self.event_sender.try_send(WidgetEvent::ButtonClick { widget_id: self.id.clone(), button: WidgetButton::Right, source: self.source.clone() });
            ...
            if *init {
                *init = false;
                ...
                if self.is_tab_button {
                    ...
                    use crate::fx::effect::selected_tab_effect;
