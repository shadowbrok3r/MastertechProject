...
async fn main() -> eframe::Result<()> {
    ...
    eframe::run_native(
        ...
        Box::new(|cc| {
            ...
            ctx.set_fonts(fonts);
            // Force dark mode: the app's theme is dark-only, but eframe follows
            // the OS theme by default and renders unreadable on light-themed
            // client PCs. Pin the preference so it never follows the system.
