...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::AuthFailCtxNotInRequestExt => {let _ = Redirect::temporary("/login"); write!(f, "{INTERNAL}")},
            Self::SurrealDb { source } => write!(f, "{source}"),
            Self::SurrealDbNoResult { id, source } => write!(f, "No result for id {id}, Source: {source}"),
            Self::SurrealDbParse { id, source } => write!(f, "Couldn't parse id {id}, Source: {source}"),
        }
    }
}
