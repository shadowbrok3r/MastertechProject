...
WHY READINGS GO MISSING: the CPU die registers and every entry in `voltages[]` need a kernel-mode access backend. `sensor_availability.backend` names the live one (`none` when nothing opened), `.backend_tier` says what it could reach (`full` / `die_only` / `rails_only` / `none`), `.backend_rejected[]` lists every backend that was tried and why it declined, and `.backend_lost` is set when a backend answered at first and then stopped — readings ended there rather than being replayed. `.detail` states all of this in one sentence; read it before concluding anything from an absent number. The two sensors reach the hardware independently, so one can read while the other does not. By default the full chain is tried in order: `pawnio`, then `mtdrv`, then `win_ring0`, then `esif_wmi` (Intel DPTF). `pawnio` leads it because it is a Microsoft attestation-signed driver, so Memory Integrity and the blocklist do not stop it; its register reach is limited to what its signed modules allow. A later backend answering does NOT mean PawnIO was absent — check `backend_rejected[]` for why it declined, since a machine reading through `win_ring0` is one PawnIO could not serve. `MTECH_LOWLEVEL_BACKEND=pawnio` opens PawnIO alone with no fallback, which is how to make that reason the only answer; `=auto` is the default chain, which needs no driver and no elevation and is why an Intel machine can report a CPU temperature with no driver loaded at all — it carries NO voltage rails, so `voltage_rails: unavailable` under it is the backend's shape, not a board fault. Memory Integrity (HVCI) and the Vulnerable Driver Blocklist only block the legacy `win_ring0` backend — `.hvci_enabled` / `.vulnerable_driver_blocklist_enabled` report them, and SetDriverProtections can turn them off (needs a reboot, and the customer's consent), but that will NOT help when the live backend is a signed one. \
