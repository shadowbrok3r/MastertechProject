use crate::{tabs::webconsole_admin::client_interface::serialize_command, virtual_filesystem::FileSysHelper, Cmd, FileSystemAction};
use database::schema::{Node, SystemInformation};
use ewebsock::{WsEvent, WsMessage};
use eframe::egui::Context;
use log::info;
use super::{deserializer, History, WebSocketClient, WsDisplayState};

impl WebSocketClient {
    pub fn receive(&mut self, ctx: &Context) {
        self.explorer.receive(ctx);
        let Ok(receiver) = self.ws_receiver.try_lock() else { return; };
                        ...
                        WsMessage::Binary(bin) => {
                            ...
                            if let Some(cmd) = deserializer::<Cmd>(bin){
                                ...
                                if msg.eq("DONE") {
                                    ...
                                    info!("Message that is ascii: {msg:?}");
                                    // Append the incoming message to the buffer with a newline
                                    self.buffer.push_str(&msg);
                                    self.buffer.push('\n');
                                } else {
                                    info!("Message not handled: {msg:?}");
                                }
                            }
                        },
                        WsMessage::Text(txt) => {
                            if txt.eq("Closed") {
                                let Ok(mut sender) = self.ws_sender.try_lock() else { return; };
                                sender.close();
                            }
                            self.loading = false;
                            info!("Text data: {txt:#?}");
        ...
        if let Ok(command) = self.send_cmd_rx.try_recv() {
            ...
            match command {
                Cmd::FileSystemAction(ref action) => {
                    match action {
                        ...
                        FileSystemAction::RequestNewContents(directory) => {
                            ...
                            sender.send(WsMessage::Binary(serialize_command(&command)));
                        }
                ...
                Cmd::Quit => {
                    ...
                    sender.send(WsMessage::Binary(serialize_command(&Cmd::Quit)));
                },
                _ => sender.send(WsMessage::Binary(serialize_command(&command)))
            }
        }
