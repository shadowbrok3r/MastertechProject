...
pub struct AgentAudit {
    ...
    loading: bool,
    status: String,
    tx: Sender<Result<Vec<AuditRow>, String>>,
    rx: Receiver<Result<Vec<AuditRow>, String>>,
}
...
/// Newest audit rows, filtered server-side so the indexes do the work.
async fn fetch(limit: usize, tool: String, failures_only: bool) -> Result<Vec<AuditRow>, String> {
    ...
    let rows: Vec<serde_json::Value> = database::db()
        ...
        .take(0)
        .map_err(|e| e.to_string())?;
    Ok(rows.iter().map(AuditRow::from_value).collect())
}
