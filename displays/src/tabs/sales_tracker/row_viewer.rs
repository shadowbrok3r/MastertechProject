...
pub struct SalesRowViewer {
    ...
    // channel back to parent to persist notes (order_id, note)
    #[serde(skip)]
    pub note_update_tx: Option<Sender<(String, String)>>,
}

impl RowViewer<SalesTableData> for SalesRowViewer {
    fn try_create_codec(&mut self, _: bool) -> Option<impl RowCodec<SalesTableData>> { Some(Codec) }
    ...
    fn column_name(&mut self, column: usize) -> std::borrow::Cow<'static, str> {
        match column {
            0 => " ID".into(),
            1 => {
                let lbl = if self.date_label.is_empty() { " Date Updated".to_string() } else { self.date_label.clone() };
    ...
    fn filter_row(&mut self, row: &SalesTableData) -> bool {
        ...
        row.order_id.to_lowercase().contains(&f)
            ...
            || row.notes.to_lowercase().contains(&f)
    }
        ...
        match column {
            ...
            9 => {
                ...
                let resp = TextEdit::singleline(&mut text)
                    ...
                    .ui(ui);
                if resp.changed() {
                    row.notes = text.clone();
                    if let Some(tx) = &self.note_update_tx { let _ = tx.try_send((row.order_id.clone(), text)); }
                }
                Some(resp)
            }
            _ => None
    ...
    fn set_cell_value(&mut self, src: &SalesTableData, dst: &mut SalesTableData, _column: usize) { *dst = src.clone(); }

    fn compare_cell(&self, l: &SalesTableData, r: &SalesTableData, column: usize) -> std::cmp::Ordering {
        ...
        match column {
            ...
            9 => l.notes.cmp(&r.notes),
            _ => Equal,
        }
    }

    fn new_empty_row(&mut self) -> SalesTableData { SalesTableData::default() }
    ...
    fn column_render_config(&mut self, column: usize, _is_last_visible_column: bool) -> TableColumnConfig {
        ...
        match column {
            ...
            9 => col.resizable(true).at_least(220.).at_most(400.),
            _ => col,
        }
    }

    fn allow_row_insertions(&mut self) -> bool { true }
}
