...
use database::schema::{TurnImage, upload_name};
...
impl Prepared {
    ...
    pub fn into_attachment(self, ctx: &egui::Context) -> Attachment {
        let thumb = self.thumb.map(|img| {
            ctx.load_texture(format!("attach:{}", self.name), img, TextureOptions::LINEAR)
        });
        Attachment {
            name: self.name,
            body: self.body,
...
pub fn encode_for_send(
    img: &DynamicImage,
    prefer_jpeg: bool,
) -> Result<(&'static str, Vec<u8>, u32, u32), String> {
    ...
    Err(format!(
        "the picture stays over {} KB even scaled down",
...
pub fn prepare(name: &str, bytes: &[u8]) -> Result<Prepared, String> {
    ...
    let Some(text) = decode_text(bytes) else {
        return Err(format!(
            "{name} is not a picture or a text file, so it cannot be attached"
        ));
    };
    if bytes.len() > TEXT_MAX_BYTES {
        return Err(format!(
            "{name} is {} KB; text files up to {} KB can be attached",
            bytes.len() / 1024,
            TEXT_MAX_BYTES / 1024
        ));
    }
...
pub fn prepare_rgba(
    ...
    let img = RgbaImage::from_raw(width, height, rgba)
        .ok_or_else(|| "the clipboard picture was malformed".to_string())?;
...
fn prepare_image(name: &str, img: &DynamicImage, prefer_jpeg: bool) -> Result<Prepared, String> {
    let (mime, bytes, width, height) =
        encode_for_send(img, prefer_jpeg).map_err(|e| format!("{name}: {e}"))?;
    Ok(Prepared {
...
pub fn split_inlined(text: &str) -> (&str, Vec<Inlined<'_>>) {
    ...
    loop {
        let Some((file, after)) = parse_inlined(rest) else {
            return (text, Vec::new());
        };
        files.push(file);
...
fn parse_inlined(s: &str) -> Option<(Inlined<'_>, &str)> {
    ...
    let close = if s.starts_with(fence) && s[ticks..].chars().next().is_none_or(|c| c == '\n') {
        ...
        let needle = format!("\n{fence}");
        let at = s.match_indices(&needle).map(|(i, _)| i).find(|i| {
            s[i + needle.len()..]
                .chars()
                .next()
                .is_none_or(|c| c == '\n')
...
fn run_off_thread(
    ...
    work: impl FnOnce() -> Result<Prepared, String> + Send + 'static,
) {
...
pub fn paste_image(ctx: &egui::Context, tx: &Sender<AttachEvent>) {
    ...
    run_off_thread(ctx, tx, || {
        let img = clipboard_bridge::read_image(std::time::Duration::from_secs(3))
            .ok_or_else(String::new)?;
        let (w, h) = (
            u32::try_from(img.width).map_err(|e| e.to_string())?,
            u32::try_from(img.height).map_err(|e| e.to_string())?,
        );
        ...
        #[cfg(target_arch = "wasm32")]
        {
            ...
            PlatformSpawner::spawn(async move {
                match file.bytes_async().await {
                    ...
                    Err(e) => {
                        let _ = tx.send(AttachEvent::Refused(format!(
    ...
    pub fn install(ctx: &egui::Context) {
        ...
        let listener = Closure::<dyn FnMut(web_sys::ClipboardEvent)>::new(
            move |event: web_sys::ClipboardEvent| {
                let Some(files) = event.clipboard_data().and_then(|d| d.files()) else {
                    return;
                };
...
mod tests {
    ...
    fn a_large_screenshot_is_sent_within_the_side_and_size_caps() {
        let (mime, bytes, w, h) =
            encode_for_send(&picture(2400, 1350, false), false).expect("encodes");
    ...
    fn noise_that_no_png_can_hold_falls_back_to_jpeg_under_the_cap() {
        let (mime, bytes, w, _) =
            encode_for_send(&picture(1600, 1600, true), false).expect("encodes");
    ...
    fn text_files_are_read_in_utf8_or_utf16_and_binaries_are_refused() {
        ...
        let le: Vec<u8> = [0xFF, 0xFE]
            .into_iter()
            .chain("REGEDIT4".encode_utf16().flat_map(u16::to_le_bytes))
            .collect();
        ...
        assert!(
            matches!(prepare("ok.log", b"line").map(|p| p.body), Ok(Body::Text(t)) if t == "line")
        );
    }
    ...
    fn composing_puts_text_files_after_the_message_and_pictures_on_the_turn() {
        ...
        assert_eq!(
            (images[0].name.as_str(), images[0].mime.as_str()),
            ("p.png", "image/png")
        );
    }
    ...
    fn inlined_files_split_back_out_of_a_sent_message() {
        ...
        assert_eq!(
            (files[0].name, files[0].lang, files[0].body),
            ("setup.log", "log", "line 1\n```\nline 3")
        );
    ...
    fn sent_picture_names_come_from_local_image_paths() {
        ...
        assert_eq!(
            image_names(Some(&item)),
            vec!["shot-0a1b2c3d.png".to_string()]
        );
