use database::{live_data::update_or_insert_layout, schema::{LiveTaskPayload, Status, Store}};
use log::{error, info};
...
impl SharedContext {
    pub fn receive_ticket(&mut self) {
        if let Ok(channel) = self.new_ticket_rx.try_recv() {
            ...
            let new_task: LiveTaskPayload = channel.new_task.1.clone(); // Convert LiveTaskPayload to TaskPayload
