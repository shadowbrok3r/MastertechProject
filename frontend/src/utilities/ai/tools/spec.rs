use crate::utilities::ai::utils::XValue;
...
pub fn tool_spec<T: JsonSchema>() -> Result<ToolSpec, Error> {
...
fn into_spec_params(mut json_schema: Value) -> Result<Value, Error> {
	...
	let properties_obj = properties
		.as_object_mut()
		.ok_or("Properties is not object").unwrap();
