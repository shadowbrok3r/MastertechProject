use async_openai_wasm::types::{ChatChoice, ChatCompletionToolChoiceOption, CreateChatCompletionRequest};
use log::info;
...
pub async fn call_with_response(input: &str) -> Result<Vec<ChatChoice>, Box<Error>> {
	...
	for tool_call in tool_calls.iter().flatten() {
		...
		let call_result = rpc_router.call_route(None, fn_name, Some(params)).await.unwrap();
