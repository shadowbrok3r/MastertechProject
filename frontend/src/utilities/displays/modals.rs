...
/// └──────────────────▼─────────────────────┘
/// ```
/// [`Modal`] is typically used via the [`ModalHandler`] helper object to reduce
/// boilerplate.
#[derive(Serialize)]
pub struct Modal {
    title: String,
...
pub struct TaskModal<'a>{
    ...
    ticket_info_page: bool,
    computer_info_page: bool,
    task_notes_page: bool,
    part_order_page: bool,
    database: Database, 
    task: &'a TaskPayload, 
    ticket_payload: &'a TicketPayload,
}
...
pub trait ModalTypes{
    /// Set the minimum width of the modal window.
    fn min_width(self, min_width: f32) -> Self;

    /// Set the minimum height of the modal window.
    fn min_height(self, min_height: f32) -> Self;

    /// Set the default height of the modal window.
    fn default_height(self, default_height: f32) -> Self;
    ...
    fn full_span_content(self, full_span_content: bool) -> Self;
...
impl ModalType{
    ...
    pub fn task_modal(&mut self, ui: &mut Ui, database: Database, task: &TaskPayload, ticket_payload: &TicketPayload){
        let mut task_modal = TaskModal::new(database, task, ticket_payload);
        task_modal.display(ui);
    }
...
impl <'a>TaskModal<'a>{
    ...
    pub fn new(database: Database, task: &'a TaskPayload, ticket_payload: &'a TicketPayload) -> Self {
        Self {
            min_width: None,
            min_height: None,
            default_height: None,
            full_span_content: false,
            ticket_info_page: false,
                    ...
                    .horizontal( |mut strip| 
                {
                    strip.cell(|ui|{
                        ui.horizontal(|ui|{
                            ...
                            if ui.selectable_label(self.computer_info_page, RichText::new("").heading()).clicked(){
                                info!("computer_info_page: {:?}", self.computer_info_page);
                            };
                        });
                    });
                    ...
                    strip.cell(|ui|
                    {
                            ...
                            .show(ui, |ui| 
                        {
                                ...
                                .show(ui, |ui| 
                            {
                                ...
                                let customer = self.ticket_payload.customer.as_ref();
                                let computer = self.ticket_payload.computer.as_ref();
                                
                                ui.label(format!("created_at: {:?}", self.ticket_payload.created_at));
                                ui.label(format!("id: {:?}", self.ticket_payload.id));
                                ui.label(format!("service_task: {:?}", self.ticket_payload.service_task));
                                ui.label(format!("service_number: {:?}", self.ticket_payload.service_number));
                                ui.label(format!("checkin_rep: {:?}", self.ticket_payload.checkin_rep));
                                ui.label(format!("sales_rep: {:?}", self.ticket_payload.sales_rep));
                                ui.end_row();
                                ui.label(format!("checkin_notes: {:?}", self.ticket_payload.checkin_notes));
                                ui.label(format!("recommendations: {:?}", self.ticket_payload.recommendations));
                                ui.label(format!("tech: {:?}", self.ticket_payload.tech));
                                ui.label(format!("salesman: {:?}", self.ticket_payload.salesman));
                                ui.label(format!("dep: {:?}", self.ticket_payload.dep));
                                ui.label(format!("terms: {:?}", self.ticket_payload.terms));
                                ui.end_row();
                                ui.label(format!("ticket_total: {:?}", self.ticket_payload.ticket_total));
                                ui.label(format!("doc_alias: {:?}", self.ticket_payload.doc_alias));
                                ui.label(format!("current_antivirus: {:?}", self.ticket_payload.current_antivirus));
