    ...
    rsx! {
        div { class: "fixed top-3 right-3 space-y-2 z-50",
            for (id, info) in mgr.read().items.clone() {
                div { class: "bg-[#0c0c10]/95 border border-[#2a2c5d]/60 rounded shadow px-3 py-2 text-sm text-slate-100 flex gap-2 items-start max-w-[280px]",
                    div { class: "flex-1",
                        if let Some(h) = info.heading.clone() { div { class: "font-semibold text-xs opacity-80", {h} } }
                        div { {info.context.clone()} }
                    }
                    button { class: "px-2 py-1 text-xs rounded border border-[#2a2c5d]/60 hover:bg-[#1e1a2a]/50",
