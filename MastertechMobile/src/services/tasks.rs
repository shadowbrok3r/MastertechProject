...
pub async fn fetch_incomplete_tasks() -> anyhow::Result<Vec<LiveTaskPayload>> {
    let tasks: Vec<LiveTaskPayload> = DATABASE
        .query("SELECT * FROM task WHERE $this.assignee.store == $store AND $this.completed IS false PARALLEL")
        .await?
        .take(0)?;
    Ok(tasks)
}
...
pub async fn fetch_completed_tasks() -> anyhow::Result<Vec<LiveTaskPayload>> {
    let tasks: Vec<LiveTaskPayload> = DATABASE
        .query("SELECT * FROM task WHERE $this.assignee.store == $store AND $this.completed IS true PARALLEL")
        .await?
        .take(0)?;
    Ok(tasks)
}
