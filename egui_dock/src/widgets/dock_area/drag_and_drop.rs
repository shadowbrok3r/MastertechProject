...
fn button_ui(
    ...
    let painter = make_overlay_painter(ui);
    painter.rect_stroke(rect, 0.0, visuals.button_border_stroke, egui::StrokeKind::Outside);
    let rect = rect.shrink(rect.width() * 0.1);
    painter.rect_stroke(rect, 0.0, button_stroke, egui::StrokeKind::Outside);
    let rim = { Rect::from_two_pos(rect.min, rect.lerp_inside(vec2(1.0, 0.1))) };
    painter.rect(rim, 0.0, visuals.button_color, Stroke::NONE, egui::StrokeKind::Outside);
