use egui::{
    ...
    CursorIcon, Frame, Id, Key, LayerId, Layout, NumExt, Order, Rect, Response, CornerRadius,
...
impl<Tab> DockArea<'_, Tab> {
    ...
    fn tab_collapse(
        ...
        let color = if response.hovered() || response.has_focus() {
            ui.painter()
                .rect_filled(rect, CornerRadius::ZERO, style.buttons.collapse_tabs_bg_fill);
    ...
    fn tab_title(
        ...
        if !is_being_dragged {
            ...
            ui.painter().hline(
                RangeInclusive::new(
                    stroke_rect.min.x + f32::max(tab_style.rounding.sw.into(), 1.5),
                    stroke_rect.max.x - f32::max(tab_style.rounding.se.into(), 1.5),
                ),
                stroke_rect.bottom(),
